...
bool Board::check_win(int last_move[2]) {
	...
	for (int reference_position = -3;reference_position < 1;reference_position++) {
		...
		if (!(win) && (reference_position + last_move[0] > -1 && reference_position + last_move[0] + TO_CONNECT - 1 < BOARD_WIDTH)) { /*checks if horizontal limits are not crossed and if another winning condition hasn't been already found*/
		...
		//vertical win check
		if (!(win) && (reference_position + last_move[1] > -1 && reference_position + last_move[1] + TO_CONNECT - 1 < BOARD_HEIGHT)) {/*checks if vertical limits are not crossed and if another winning condition hasn't been already found*/
			win = true;
			for (int n = 1;n < TO_CONNECT;n++) {
		...
		if (!(win) && (reference_position + last_move[1] > -1 && reference_position + last_move[1] + TO_CONNECT - 1 < BOARD_HEIGHT) && (reference_position + last_move[0] > -1 && reference_position + last_move[0] + TO_CONNECT - 1 < BOARD_WIDTH)) { /*checks if  limits are not crossed and if another winning condition hasn't been already found*/
			win = true;
			for (int n = 1;n < TO_CONNECT;n++) {
		...
		if (!(win) && (-reference_position + last_move[1] < BOARD_HEIGHT && -reference_position + last_move[1] - TO_CONNECT + 1 >-1) && (reference_position + last_move[0] > -1 && reference_position + last_move[0] + TO_CONNECT - 1 < BOARD_WIDTH)) { /*checks if limits are not crossed and if another winning condition hasn't been already found*/
			win = true;
			for (int n = 1;n < TO_CONNECT;n++) {
				if ((slots[reference_position + last_move[0]][-reference_position + last_move[1]] != slots[reference_position + last_move[0] + n][-reference_position + last_move[1] - n])) {
					win = false;
